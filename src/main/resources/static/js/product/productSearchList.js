(function () {
    ...
    function updatePageInfo(total = null) {
        ...
        if (state.brand) {
            pageTitle.textContent = `${state.brand} 전체상품${total != null ? ' (' + total + '개)' : ''}`;
            document.title = `${state.brand} 전체상품 - PERSOME`;
        } else if (state.keyword && state.keyword !== 'none') {
    ...
    function qs(params) {
        ...
        // 브랜드 검색
        if (state.brand) {
            p.set('name', state.brand);
            return `brand?${p.toString()}`;
        }
    ...
    function renderCount(p) {
        ...
        if (state.brand) {
            ...
            $cnt.textContent = `${state.brand}에 대한 ${total}개의 상품이 등록되어 있습니다.`; // ✅ 브랜드 문구
        } else {
            updatePageInfo(total);
            ...
            btn.addEventListener('click', () => {
                ...
                if (!isNaN(to)) {
                    ...
                    if (urlParams.get('name')) {
                        state.brand = urlParams.get('name'); // 브랜드 유지
                    }
                    load();
                }
            });
